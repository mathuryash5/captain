...
from qa_system.forms import RegistrationForm, LoginForm, UpdateAccountForm, PostForm, RequestResetForm, ResetPasswordForm, AddQuestionsForm
from qa_system.models import User, Post, Questions
from flask_login import login_user, current_user, logout_user, login_required
from flask_mail import Message
from qa_system.cluster_knowledge import *
from qa_system.answer_questions import _answer_questions
...
@app.route("/post/new/form", methods=['GET', 'POST'])
@login_required
def new_post():
    global post_count
    form = PostForm()
    if form.validate_on_submit():
        post = Post(id=post_count-1, title=form.title.data, content=form.title.content, author=current_user)
        db.session.add(post)
        db.session.commit()
        flash('Your post has been created!', 'success')
        return redirect(url_for('home'))
    return render_template('create_post.html', title='New Topic',form=form, legend='New Topic')


@app.route("/post/<int:post_id>/show_pdf")
def show_pdf(post_id):
    return Response(open("/uploads/" + str(post_id) + ".pdf" , "rb").read(), mimetype = "application/pdf")

@app.route("/post/<int:post_id>/show_txt")
def show_txt(post_id):
    return Response(open("/uploads/" + str(post_id) + ".txt" , "rb").read(), mimetype = "application/txt")
...
@app.route("/post/<int:post_id>/add_questions", methods = ['GET', 'POST'])
@login_required
def add_questions(post_id):
    ...
    if form.validate_on_submit():
        questions = Questions(questions = form.questions.data, post_id = post_id)
        db.session.add(questions)
        db.session.commit()
        flash('Your questions have been added!', 'success')
        return redirect(url_for('home'))
    return render_template('add_questions.html', title='Add Questions',
                           form=form, legend='Add Questions')
...
@app.route("/post/<int:post_id>/answer_question", methods = ["GET","POST"])
@login_required
def answer_question(post_id):
    ...
    question = "What is true of a DFA"
    options = ["FSM can have one or more final states", "FSM can have more than one start state", "Input alphabet can be infinite", "Language should be finite"]
    topic_id = post.id
    
    words = pickle.load(open(str(topic_id+1) + '_Important_Words', 'rb'))
    search_matrix_sparse = pickle.load(open(str(topic_id+1) + '_Search_Matrix_Sparse', 'rb'))
    clusters = pickle.load(open(str(topic_id+1) + '_Clusters', 'rb'))
    sentences = pickle.load(open(str(topic_id+1) + '_Sentences', 'rb'))
        
    sentence = sent_tokenize(question)
    cleaned_sentence = [CleanSentence(_sentence) for _sentence in sentence]
    
    query_vector = numpy.zeros(shape = (len(words),))

    query_words = set([word for _sentence in cleaned_sentence for word in _sentence])
    for word in query_words:
        try:
            query_vector[words.index(word)] = 1
        except:
            pass

    cluster_number = numpy.argmax(search_matrix_sparse * query_vector)      

    cluster_sentences_index = clusters[cluster_number]      
...
@app.route("/post/new/upload_pdf", methods = ["POST"])
def upload_pdf():
    global post_count
    #_temp = "<a href= '/post/" + str(post_count-1) + "/show_pdf" + "'></a>"
    _temp = "Pdf Uploaded"
    post = Post(id=post_count-1, title=request.form['title'], content=_temp, author=current_user)
    db.session.add(post)
    db.session.commit()
    ...
    if file:
        if allowed_file(file.filename):
            ...
            file.save(os.path.join(app.config['UPLOAD_FOLDER'], str(topic_id) + ".pdf"))

            string = extract_string_from_pdf(os.path.join(app.config['UPLOAD_FOLDER'], str(topic_id) + ".pdf"))
...
@app.route("/post/new/upload_txt", methods = ["POST"])
def upload_txt():
    topic_id = str(post_count)
    _temp = "<a href= '/post/" + str(post_count-1) + "/show_txt" + "'></a>"
    post = Post(id=post_count-1, title=request.form['title'], content=_temp, author=current_user)
    db.session.add(post)
    db.session.commit()
    post_count += 1
    ...
    if file:
        if allowed_file(file.filename):
            filename = secure_filename(file.filename)
            file.save(os.path.join(app.config['UPLOAD_FOLDER'], str(topic_id) + ".txt"))
...
@app.route("/post/new/upload_inputted_txt", methods = ["POST"])
def upload_inputted_txt():
    global post_count
    topic_id = str(post_count)
    post_count += 1

    string = dict(request.form)['content']
